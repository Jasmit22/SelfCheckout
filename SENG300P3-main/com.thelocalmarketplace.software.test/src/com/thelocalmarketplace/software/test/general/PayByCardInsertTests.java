...
import com.thelocalmarketplace.software.general.Enumerations;
 ...
 */
public class PayByCardInsertTests {
    ...
    public void setup() {
        ...
        //set up bank details
        bank = new CardIssuer("UBS",3); // <--- nur weil ich kann auch!
        ...
        this.session.selectPaymentMethod(Enumerations.PaymentMethods.DEBIT);
        session.getProductLogic().updateBalance(BigDecimal.valueOf(10.00));
        session.getHardware().getCardReader().enable();
        session.getStateLogic().gotoState(Enumerations.States.CHECKOUT);
    }

/**
 * NOTE: No error handling/testing for bronze and silver checkout stations
 */
    ...
    public void testInvalidState() throws IOException {
        session.getProductLogic().updateBalance(BigDecimal.valueOf(10.00));
        session.getStateLogic().gotoState(Enumerations.States.NORMAL);
        session.getHardware().getCardReader().insert(this.card, this.pin);
    ...
    /**
     * Test if a card is properly blocked after trying to use it 3 times with an invalid pin.
     * The only change in the system should be the card being blocked.
     * Everything else should function normally after a card is blocked
     * @throws IOException
     */
    @Test 
    public void testBlockingCardAfterThreeFails() throws IOException {
    	//Try to insert the card with the wrong pin 3 times. The hardware should throw a wrong PIN exception only.
    	for(int i=0; i<3; i++) {
    		try{
    			session.getHardware().getCardReader().insert(this.card, this.wrongPIN);
    			fail();
    		}
    		catch(Exception e){
    			System.out.println(e.toString());
    			assertTrue(e instanceof InvalidPINException);
    		}
    		// remove the card
    		session.getHardware().getCardReader().remove();
    	}
    	// ensure the balance owed has not been reduced
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
        // Try the card again with the wrong pin. Should instead give a BlockedCardException as the card should be blocked.
        try{
        	session.getHardware().getCardReader().insert(this.card, this.wrongPIN);
        	fail();
        }
        catch(Exception e){
        	assertTrue(e instanceof BlockedCardException);
        }
        // remove the card
        session.getHardware().getCardReader().remove();
        // try using the card with the actual PIN. Same should happen, as the card is blocked.
        try{
        	session.getHardware().getCardReader().insert(this.card, this.wrongPIN);
        	fail();
        }
    ...
    public void testWrongPaymentMethodSelected() throws IOException {
        session.getProductLogic().updateBalance(BigDecimal.valueOf(10.00));
        session.selectPaymentMethod(Enumerations.PaymentMethods.CREDIT);
        session.getHardware().getCardReader().insert(this.card, this.pin);
    ...
    public void testStationBlockedInsert()throws IOException {
        ...
        session.getStateLogic().gotoState(Enumerations.States.NORMAL);
        session.getStateLogic().gotoState(Enumerations.States.BLOCKED);
        session.getHardware().getCardReader().insert(this.card, this.pin);
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
    }
    ...
    public void testNullCard() throws IOException {
        session.getHardware().getCardReader().insert(null, this.pin);
    }

    /**
     * Test to ensure that a valid membership card is registered and that card insertion does not prevent anything.
     * @throws IOException
     */
    @Test
    public void testMembershipTransaction() throws IOException {
        Card membershipCard = new Card("MEMBERSHIP","111234567","Joe","319", pin, true, true);
        session.getMembershipLogic().registerMembershipToLocalDatabase("111234567");
        session.getStateLogic().gotoState(Enumerations.States.NORMAL);
        session.getStateLogic().gotoState(Enumerations.States.ADDMEMBERSHIP);
    ...
    public void testBadCard() throws IOException {
        Card badCard = new Card("DEBIT","987654321","Joe","319", "101", true, true);
        session.getHardware().getCardReader().insert(badCard, "101");
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
    }

    /**
     * Test to ensure that a false card is rejected... Bad name
     * @throws IOException
     */
    //@Test
    public void testBadCard2() throws IOException {
        Card badCard = new Card("DEBIT","123456789","Joe","319", this.pin, true, true);
        session.getHardware().getCardReader().insert(badCard, this.pin);
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
    }

    /**
     * Test to ensure that an unassigned card is rejected... Bad CVV
     * @throws IOException
     */
    //@Test
    public void testBadCard3() throws IOException {
        Card badCard = new Card("DEBIT","123456789","John","318", this.pin, true, true);
        session.getHardware().getCardReader().insert(badCard, this.pin);
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
    }

    /**
     * Test to ensure that an unassigned card is rejected... Bad PIN
     * @throws IOException
     */
    @Test(expected = InvalidPINException.class)
    public void testBadCard4() throws IOException {
        session.getHardware().getCardReader().insert(this.card, "101");
    }
    /**
     * Test to ensure that an insert disabled card is rejected...
     * @throws IOException
     */
    @Test(expected = ChipFailureException.class)
    public void testBadCard5() throws IOException {
        Card badCard = new Card("DEBIT","123456789","John","319", this.pin, true, false);
        session.getHardware().getCardReader().insert(badCard, this.pin);
        assertEquals(BigDecimal.valueOf(10.0), session.getProductLogic().getBalanceOwed());
    }
